...
BOOL CSDEvent::CreateSdEvent()
{
	...
	if ( m_pEvent == NULL )
	{
		SDLOG_PRINTF_U(0, SD_LOG_LEVEL_ERROR, "SDEvent:%s create failed!", m_strEventName);
		return FALSE;
	}
#else
	int nRet = 0;
	...
	if (nRet != 0)
	{
		SDLOG_PRINTF_U(0, SD_LOG_LEVEL_ERROR, "SDEvent:%s create failed! pthread_mutex_init failed return:%d!", 
					m_strEventName, nRet);
		return FALSE;
	}
	...
	if (nRet != 0)
	{
		SDLOG_PRINTF_U(0, SD_LOG_LEVEL_ERROR, "SDEvent:%s create failed! pthread_cond_init failed return:%d!", 
					m_strEventName, nRet);
...
BOOL CSDEvent::waittime(int milliseconds)
{
	...
	{
		SDLOG_PRINTF_U(0, SD_LOG_LEVEL_ERROR, "SDEvent:%s waittime failed ret:%d!", m_strEventName, ulRet);
		ResetEvent(m_pEvent);
		return FALSE;
	}
#else
...
BOOL CSDEvent::post()
{
	...
	if (m_pEvent == NULL)
	{
		SDLOG_PRINTF_U(0, SD_LOG_LEVEL_ERROR, "SDEvent:%s post failed point is null!!", m_strEventName);
		return FALSE;
	}
	...
	if (nRet == FALSE)
	{
		SDLOG_PRINTF_U(0, SD_LOG_LEVEL_ERROR, "SDEvent:%s SetEvent failed!!", m_strEventName);
	}
	return nRet;
#else
	int nRet = 0;
	...
	{
		SDLOG_PRINTF_U(0, SD_LOG_LEVEL_ERROR, "SDEvent:%s SetEvent failed return:%d!!", m_strEventName, nRet);
		return FALSE;
	}
#endif
}

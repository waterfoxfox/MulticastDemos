...
BOOL CSDThread::CreateThread(CallBack2 sdCallBack, CallBack2 pThreadExit, void* pParam1, void* pParam2)
{
	...
	{
		SDLOG_PRINTF_U(0, SD_LOG_LEVEL_ERROR, "CreateThread:%s failed, status:%d!", m_strName, m_nStatus);
		return FALSE;
	}
	...
	if ( m_pth == NULL )
	{
		SDLOG_PRINTF_U(0, SD_LOG_LEVEL_FATAL, "CreateThread:%s failed!", m_strName);
		m_sdCallBack2 = NULL;
		m_pParam2 = NULL;
		m_pParam1 = NULL;
		return FALSE;
	...
	if( ThreadID != 0 )
	{
		SDLOG_PRINTF_U(0, SD_LOG_LEVEL_FATAL, "Create pthread:%s error!", m_strName);
		m_sdCallBack2 = NULL;
		m_pParam2 = NULL;
		m_pParam1 = NULL;
		return FALSE;
...
BOOL CSDThread::CreateThread(CallBack1 sdCallBack, CallBack1 pThreadExit, void *pParam)
{
	...
	if ( sdCallBack == NULL ||
		 ...
		 m_nStatus == THRD_STATUS_RUN)
	{
		SDLOG_PRINTF_U(0, SD_LOG_LEVEL_FATAL, "CreateThread:%s failed, status:%d!", m_strName, m_nStatus);
		return FALSE;
	}	
	...
	if ( m_nStatus == THRD_STATUS_RUN)
	{
		if ( m_pth != NULL )
		{
			...
			if(m_nStatus != THRD_STATUS_CLOSE)
			{
				...
				SDLOG_PRINTF_U(0, SD_LOG_LEVEL_FATAL, "Thread:%s force close is call,force the running thread out!", m_strName);
				bRet = FALSE;
			}
			else
			{
		...
		{
			SDLOG_PRINTF_U(0, SD_LOG_LEVEL_ERROR, "Thread:%s is not closed,but m_pth is NULL!", m_strName);
		}
	}
